# Standardize Python-Rust FFI Boundary with PyO3 for External API Validation: Configuration Objects Validators
...
1. SHOULD: Configuration objects (validators/config.rs) SHOULD be immutable once passed across the FFI boundary to prevent race conditions

## Policy Block

- SHOULD Configuration objects (validators/config.rs) SHOULD be immutable once passed across the FFI boundary to prevent race conditions
