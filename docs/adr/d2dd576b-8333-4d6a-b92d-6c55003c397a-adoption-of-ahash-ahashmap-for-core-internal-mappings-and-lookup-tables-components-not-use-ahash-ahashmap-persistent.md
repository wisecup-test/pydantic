...
1. MUST_NOT: Components MUST NOT use ahash::AHashMap for persistent or cross-process serialization formats where deterministic cross-platform hash values or cryptographic collision resistance against untrusted adversarial key inputs is strictly required.

## Policy Block

- MUST_NOT Components MUST NOT use ahash::AHashMap for persistent or cross-process serialization formats where deterministic cross-platform hash values or cryptographic collision resistance against untrusted adversarial key inputs is strictly required.
