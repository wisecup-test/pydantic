...
1. MUST: Before implementing or modifying datetime parsing logic relying on external parsing crates, consumers MUST locate the repository dependency lock artifact to determine the exact resolved version and verify the public contract of the dependency against official release documentation.

## Policy Block

- MUST Before implementing or modifying datetime parsing logic relying on external parsing crates, consumers MUST locate the repository dependency lock artifact to determine the exact resolved version and verify the public contract of the dependency against official release documentation.
