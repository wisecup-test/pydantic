...
1. MUST_NOT: Serializer modules MUST_NOT bypass the serde::ser::Error trait by returning raw unformatted strings or foreign error types that fail to implement the serialization framework error contract.

## Policy Block

- MUST_NOT Serializer modules MUST_NOT bypass the serde::ser::Error trait by returning raw unformatted strings or foreign error types that fail to implement the serialization framework error contract.
