...
1. MUST: Type serializers coordinating shared cross-thread or definitions state MUST combine std::borrow::Cow with thread-safe pointer structures rather than duplicating owned buffers across serializing routines.

## Policy Block

- MUST Type serializers coordinating shared cross-thread or definitions state MUST combine std::borrow::Cow with thread-safe pointer structures rather than duplicating owned buffers across serializing routines.
