...
1. MUST: Before introducing or updating collections backed by ahash::AHashMap, the consumer MUST inspect the repository lock artifact to determine the exact resolved dependency version and verify that all invoked constructors and collection methods match that resolved version specification.

## Policy Block

- MUST Before introducing or updating collections backed by ahash::AHashMap, the consumer MUST inspect the repository lock artifact to determine the exact resolved dependency version and verify that all invoked constructors and collection methods match that resolved version specification.
