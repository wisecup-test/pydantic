...
1. MUST_NOT: Internal proxy mocker classes for validators and serializers MUST NOT perform eager schema compilation if the target schema requires deferred rebuilding, and MUST raise designated user errors when rebuild triggers fail.

## Policy Block

- MUST_NOT Internal proxy mocker classes for validators and serializers MUST NOT perform eager schema compilation if the target schema requires deferred rebuilding, and MUST raise designated user errors when rebuild triggers fail.
