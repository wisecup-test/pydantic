...
1. MUST: Internal metadata inspection utilities MUST reconstruct type expressions by wrapping the core annotation with `annotated_types` metadata objects.

## Policy Block

- MUST Internal metadata inspection utilities MUST reconstruct type expressions by wrapping the core annotation with `annotated_types` metadata objects.
