# Implement Cache Layer for External API Data Validation: Cache Size Bounded
...
1. SHOULD: Cache size SHOULD be bounded with an eviction policy (LRU or similar) to prevent unbounded memory growth

## Policy Block

- SHOULD Cache size SHOULD be bounded with an eviction policy (LRU or similar) to prevent unbounded memory growth
