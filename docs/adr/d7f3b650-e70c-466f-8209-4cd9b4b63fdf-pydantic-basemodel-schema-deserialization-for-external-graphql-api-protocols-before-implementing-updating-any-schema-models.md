...
1. MUST: Before implementing or updating any schema models that depend on an external serialization library, the developer MUST inspect the authoritative lock artifact in the repository to resolve the exact dependency version and verify API compatibility.

## Policy Block

- MUST Before implementing or updating any schema models that depend on an external serialization library, the developer MUST inspect the authoritative lock artifact in the repository to resolve the exact dependency version and verify API compatibility.
