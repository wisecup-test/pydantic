...
1. MUST: Lifecycle hooks requiring external service credentials for execution MUST access them through environment mappings and raise PluginError when mandatory configuration prerequisites are absent.

## Policy Block

- MUST Lifecycle hooks requiring external service credentials for execution MUST access them through environment mappings and raise PluginError when mandatory configuration prerequisites are absent.
