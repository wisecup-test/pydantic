# Implement Cache Layer for External API Data Validation: Cache Implementations Not
...
1. MUST_NOT: Cache implementations MUST NOT cache validation results that depend on external state that may change frequently (e.g., real-time DNS availability checks)

## Policy Block

- MUST_NOT Cache implementations MUST NOT cache validation results that depend on external state that may change frequently (e.g., real-time DNS availability checks)
