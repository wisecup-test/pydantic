...
1. MUST: Initialization of values managed by std::sync::OnceLock MUST occur via get_or_init or get_or_try_init to guarantee single-execution semantics across concurrent threads.

## Policy Block

- MUST Initialization of values managed by std::sync::OnceLock MUST occur via get_or_init or get_or_try_init to guarantee single-execution semantics across concurrent threads.
