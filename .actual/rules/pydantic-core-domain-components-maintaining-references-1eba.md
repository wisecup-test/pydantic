# pyo3 Traversal and Conversion Protocol Integration for Domain Models: Domain Components Maintaining References Host Runtime

These rules are ALWAYS ACTIVE for domain model definitions, type serializers, schema validators, lookup structures, and domain structures maintaining references to host runtime objects across runtime boundaries.

### Rules

- **R-PYO3-001** MUST: Domain components maintaining references to host runtime objects MUST implement visitor traversal using PyGcTraverse with PyVisit and PyTraverseError to participate in cycle detection.
