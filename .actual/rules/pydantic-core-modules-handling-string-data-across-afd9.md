# PyO3 PyBackedStr Adoption for Zero-Copy String References in Core FFI Pipelines: Modules Handling String Data Across Foreign
...
- **R-PYO3-001** MUST: Modules handling string data across the foreign function interface boundary MUST adopt pyo3::pybacked::PyBackedStr to retain references to foreign string buffers without performing eager byte-copying heap allocations.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Verification is enforced via continuous integration static analysis pipelines, peer code review for foreign function interface data paths, and automated benchmark regression suites.
</enforcement>