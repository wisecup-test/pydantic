# Standardize External Client Boundary Patterns in Internal APIs: Boundary Layers Implement
...
- **R-BOUND-001** SHOULD: API boundary layers SHOULD implement validation and transformation logic to protect internal implementations from invalid external inputs.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Automated CI checks MUST scan for imports of _internal modules from external code. Code review MUST include boundary pattern compliance verification. Static analysis tools MUST be configured to flag direct access to internal APIs. Violations MUST result in CI pipeline failure and code review blocks unless documented exceptions are approved by the architecture team.
</enforcement>