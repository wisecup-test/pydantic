<rule_activation id="9970f366-be59-41a5-9540-734eb1e1b4dd" title="Enforce Domain Validation in Public API Models: Models Use Root" applies_to="**/*">
These rules are ALWAYS ACTIVE for all public/external API endpoints and data models exposed to external consumers.
</rule_activation>

### Rules

- **R-API-001** SHOULD: API models SHOULD use root models or base model inheritance to ensure consistent validation behavior across all endpoints.
...
<enforcement>
Claude Code MUST verify all rules in this activation block. Type checking and validation framework compliance is mandatory for all public API models. Verification via mypy strict mode and test coverage is non-negotiable.
</enforcement>