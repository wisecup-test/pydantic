<rule_activation id="54d5e18b-12eb-49b7-b259-a45300405f65" title="Adopt Type Annotation and Runtime Validation for Public API Interfaces: Public Functions Methods" applies_to="**/*">
...
- **R-API-001** MUST: All public API functions, methods, and classes MUST include complete type annotations for parameters, return values, and class attributes.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. CI/CD pipeline MUST run mypy in strict mode on all public API modules and fail builds on type checking errors. Code review MUST include verification of type annotations and runtime validation for all API changes. Violation handling includes blocking pull requests that fail mypy strict mode checks and requiring explicit justification for any `type: ignore` suppressions.
</enforcement>