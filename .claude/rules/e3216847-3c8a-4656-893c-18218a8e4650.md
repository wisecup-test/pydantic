<rule_activation id="e3216847-3c8a-4656-893c-18218a8e4650" title="Enforce Domain Validation in Public API Models: Models Type Checked" applies_to="**/*">
These rules are ALWAYS ACTIVE for all public/external API endpoints and data models exposed to external consumers.
</rule_activation>

### Rules

- **R-DOMAIN-001** MUST: All API models MUST be type-checked using static analysis tools (mypy, pyright) as part of the CI/CD pipeline.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Type checking and validation are mandatory for all public API models. CI/CD pipeline MUST fail if mypy reports type errors in API modules. Pull requests MUST be blocked until validation requirements are met.
</enforcement>